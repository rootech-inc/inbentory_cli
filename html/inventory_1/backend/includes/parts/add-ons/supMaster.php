<?php
    ...
    $suppliers = $db->db_connect()->query("SELECT * FROM supp_mast");
...
<body style="height: 100vh; overflow: hidden">
    <header class="p-2">
        <button data-toggle="modal" onclick="prod_master.CreateSupplier()" class="btn btn-info rounded-0 btn-sm">New Supplier</button>
        ...
        <table class="table table-sm table-hover">
            ...
            <tbody id="taxScreen">
                <?php while ($supplier = $suppliers->fetch(PDO::FETCH_ASSOC)): ?>
                    <tr>
                        <td><?php echo $supplier['supp_id'] ?></td>
                        <td><?php echo $supplier['supp_name'] ?></td>
                        <td></td>
                    </tr>
                <?php endwhile; ?>
            </tbody>
